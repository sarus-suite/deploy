...
function get_artifacts_versions() {
  unset SARUSCTL_VERSION
  SARUSCTL_VERSION="v$(${BIN} --version | awk '/sarusctl /{print $NF}')"
}

function check_artifacts_versions() {
  if [ -z "${SARUSCTL_VERSION}" ]
  then
    echo "Error: Cannot find \$SARUSCTL_VERSION, fetch sarusctl in advance."
    return 1
  fi
}
